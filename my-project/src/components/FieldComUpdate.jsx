...
const FieldComUpdate = ({ show, setshow, title, data }) => {
  ...
  let findUpdateData = sectors.find((e) => e._id == data);

  const [name, setname] = useState(findUpdateData?.name);

  const [selectedSubsector, setSelectedSubsector] = useState(
    findUpdateData?.subsector
  );
  const [selectedMicroSubsector, setselectedMicroSubsector] = useState(
    findUpdateData?.microsector
  );
  const [selectedSector, setSelectedSector] = useState(findUpdateData?.sector);

  const [agree, setagree] = useState(true);

  // console.log(selectedMicroSubsector, selectedSubsector, selectedSector, name);
    ...
    return (
      <div className="px-4 space-y-2">
        <select
          ...
          defaultValue={findUpdateData?.sector}
        ...
        {selectedSector && (
          <select
            ...
            defaultValue={findUpdateData?.subsector}
          >
